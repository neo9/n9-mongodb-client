...
test('[Indexes] Create expiration index', async (t: Assertions) => {
	...
	t.true(index.key && index.key['objectInfos.creation.date'] === 1, 'index exists and is on `objectInfos.creation.date` field');
...
test('[Indexes] Create historic expiration index', async (t: Assertions) => {
	...
	const index = (await historicCollection.listIndexes().toArray()).find((i) => i.name === 'expiration');
	t.truthy(index, 'index exists and is named expiration');
...
test('[Indexes] Update expiration index if it exists', async (t: Assertions) => {
	...
	t.true(index.key && index.key['objectInfos.lastUpdate.date'] === 1, 'index exists and is on `objectInfos.lastUpdate.date` field');
...
test('[Indexes] Update historic expiration index if it exists', async (t: Assertions) => {
	...
	await mongoClient.createHistoricExpirationIndex(1, 'date', { name: 'expiration' });
	let index = (await historicCollection.listIndexes().toArray()).find((i) => i.name === 'expiration');
	t.true(index.expireAfterSeconds === 86400, 'index exists with initial ttl');
