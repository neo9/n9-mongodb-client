...
test('[LOCK-FIELDS-ARRAY A] Import, remove one, import others, import new one', async (t: Assertions) => {
	...
	// operator update
	const newValue2 = await mongoClient.findOneAndUpdateByIdWithLocks(resultImport1[0]._id, vAp, 'userIdUpdate', true, true);
	t.deepEqual(_.map(newValue2.parameters.items, 'code'), ['a', 'b'], '[newValue2] Check value updated');
	...
	t.deepEqual(_.map(resultImport2[0].objectInfos.lockFields, 'path'), aAndbLockPaths, '[resultImport2] a and b are locked');
	t.deepEqual(_.map(resultImport2[0].parameters.items, 'code'), ['a', 'b', 'c'], '[resultImport2] Check value updated');
	...
	t.deepEqual(_.map(resultImport3[0].parameters.items, 'code'), ['a', 'b', 'c', 'd'], '[resultImport3] Check value updated');
	...
	t.deepEqual(_.map(resultImport4[0].parameters.items, 'code'), ['a', 'b', 'd'], '[resultImport4] Check value updated');
});
